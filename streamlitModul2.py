    ...
    if st.button("Train Model"):
        ...
        for label_col in label_columns[:3]:  # Just showing first 3 for clarity
            ...
            comparison_df[f"{label_col}_match"] = (
                comparison_df[f"{label_col}_actual"]
                == comparison_df[f"{label_col}_predicted"]
            )
            ...
            for col in range(3):
                ...
                if label_idx < len(label_columns):
                    with cols[col]:
                        ...
                        sns.heatmap(
                            mcm[label_idx], annot=True, fmt="d", cmap="Blues", ax=ax
                        )
                        plt.title(f"Confusion Matrix: {label_columns[label_idx]}")
